...
import { Flame, Droplets, Beef, Wheat, Nut, Sparkles, Wheat as FiberIcon, Trophy } from "lucide-react";
      ...
      <div className="mx-auto max-w-md px-5 pt-14">
        ...
        <div className="glass-strong mt-4 rounded-[28px] p-5">
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2"><Droplets className="h-4 w-4 text-hydration" /><span className="text-sm font-semibold">{t("dashboard.hydration")}</span></div>
              <div className="mt-1 flex items-baseline gap-1">
            ...
            <div className="relative h-24 w-14 overflow-hidden rounded-[20px] border-2 border-hydration/40 bg-hydration/5">
              <motion.div animate={{ height: `${Math.min(100, (totals.waterMl / goals.waterMl) * 100)}%` }} transition={{ type: "spring", damping: 22 }} className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-hydration to-hydration/60" />
            </div>
          </div>
          <div className="mt-4 grid grid-cols-4 gap-2">
            {[125, 250, 375, 500].map((ml) => (
              <motion.button key={ml} whileTap={{ scale: 0.94 }} onClick={() => mutate.mutate(ml)}
                ...
                <span className="text-[10px] font-semibold">+{ml}ml</span>
              </motion.button>
            ))}
          </div>
        </div>
